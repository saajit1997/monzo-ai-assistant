...
**Phase 7 — RAG assistant.** Phases 1–3 built the content pipeline: sitemap → URL inventory (`data/raw/monzo_urls.csv`) → raw HTML (`data/raw/pages/`, manifest at `data/raw/pages_manifest.csv`) → clean structured text (`data/processed/pages.parquet`). Phase 6 chunks that text and builds a hybrid retrieval index (local sentence-transformers + FAISS + BM25) under `data/processed/retrieval/`. Phase 7 wires that retrieval into an actual chat app — `app/streamlit_app.py` — that calls Claude (Haiku 4.5) to generate a grounded answer with citations, and logs every question, its retrieved chunks, and the answer to a local SQLite database (`data/analytics/query_log.db`) — the groundwork for Phase 8/9's product analytics. Phases 4 (dbt over static content) and 5 (knowledge graph) stay deferred — see the MVP scope note in `docs/roadmap.md`.
...
- All of `data/analytics/` — including Phase 7's `query_log.db` — is **not committed** (`.gitignore`'d, `.gitkeep` aside). This is runtime usage data generated by people actually using the app, not a build artifact; it doesn't belong in git regardless of size.
