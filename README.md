...
```text
Monzo Public Website → Data Ingestion → Content Processing → Vector Retrieval
    → LLM/RAG → AI Assistant → Query Analytics → Product Insights
```
...
**Phase 5 — RAG assistant.** Phases 1–3 built the content pipeline: sitemap → URL inventory (`data/raw/monzo_urls.csv`) → raw HTML (`data/raw/pages/`, manifest at `data/raw/pages_manifest.csv`) → clean structured text (`data/processed/pages.parquet`). Phase 4 chunks that text and builds a hybrid retrieval index (local sentence-transformers + FAISS + BM25) under `data/processed/retrieval/`. Phase 5 wires that retrieval into an actual chat app — `app/streamlit_app.py` — that calls Claude (Haiku 4.5) to generate a grounded answer with citations, and logs every question, its retrieved chunks, and the answer to a local SQLite database (`data/analytics/query_log.db`) — the groundwork for Phase 6/7's product analytics. Deployed live on Hugging Face Spaces.
...
| 4 | Embeddings + hybrid retrieval |
| 5 | RAG assistant *(current)* |
| 6 | Query/event analytics |
...
- `data/processed/retrieval/` (Phase 4's FAISS index + chunk table) **is committed to git.** Small (~3MB) and, unlike Phase 3's output, *is* regenerable from something already committed (`pages.parquet`) — committed anyway so reviewers can query the assistant's retrieval step immediately instead of waiting ~30s for a local embedding build.
- All of `data/analytics/` — including Phase 5's `query_log.db` — is **not committed** (`.gitignore`'d, `.gitkeep` aside). This is runtime usage data generated by people actually using the app, not a build artifact; it doesn't belong in git regardless of size.
- The rest of `data/processed/` is likewise not committed. Future phases may generate much larger derived content there.
...
Also try `python scripts/serve_search_ui.py` — a free, retrieval-only local test UI (no LLM call, no API key needed) for judging retrieval quality directly.

## Running Phase 5

```bash
# Add your own key to a local .env first — see .env.example
streamlit run app/streamlit_app.py
```

Requires `ANTHROPIC_API_KEY` in a local `.env` (get one at [console.anthropic.com](https://console.anthropic.com/)). Unlike every earlier phase, this one costs money per question — Claude API calls aren't free. Costs are trivial for personal use (Haiku 4.5, short grounded answers), but it's a genuine departure from the rest of the project, which runs entirely free and offline.

The app retrieves context with Phase 4's hybrid search, asks Claude (`claude-haiku-4-5` by default, configurable in `config/sources.yaml`) to answer *only* from that context, and shows the answer with a "Sources" citation and an expandable list of the chunks retrieval considered. If retrieval doesn't find a good answer, the assistant says so explicitly rather than guessing — every exchange (question, retrieved chunks, answer, whether it actually answered) is logged to `data/analytics/query_log.db` for future analytics.
